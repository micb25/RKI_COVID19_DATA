...
n = -STATS_min
f(x) = m * x + n
fit [ xmax - 14 * 86400 : xmax ] f(x) "<awk -F, '{ if ((NR>1)&&($3==".REGION_ID.")) print $1, $11}' ../RKI_COVID19_Impfquotenmonitoring.csv" using ($1):(100*$2/POPULATION) via m, n
...
if ( vac_per_day_rel > 0 ) {
	set label 3 at graph 0.03, 0.71 "{/:Bold 14-Tage-Trend (Zweitimpfungen):}" left textcolor ls 10
